...
def _get_node_info(args):
	return get_node_info(args[0], args[1]) #TODO: Current retry_api do not support this type of output
...
def get_node_list(self): # TODO: testing
	...
	while(not get_node_list_cli.wait_for_service(timeout_sec=2.0)): # Wait for service to start
		self.get_logger().info("Service not available, trying again...")
	...
	if(future.done()):
		try:
			...
			if(response.status == response.ERROR or response.status == response.FATAL): # get_node_list service doesn't support this right now
				raise Exception
		except Exception as e:
			self.get_logger().error("Error occured %r"%(e,))
