...
class ArmManager(Node):
	...
	def __init__(self, name):
		...
		self.load_transfer_service = self.create_service(LoadTransfer, "/arm/%s/load_transfer"%self.id, self.load_transfer_handler)
		self.get_next_transfer_service = self.create_service(GetNextTransfer, "/arm/%s/get_next_transfer"%self.id, self.get_next_transfer_handler)
		...
		self.completed_transfer_sub = self.create_subscription(CompletedTransfer, "/arm/%s/completed_transfer"%self.id, self.completed_transfer_callback, 10)
	...
	def completed_transfer_callback(self, msg):
		...
		self.get_logger().info("Completed transfer " + str(self.completed_queue)) #TODO: DELETE
...
#		self.get_logger().info("both ready %s, in queue %s"%(str(both_ready), str(in_queue))) #TODO: DELETE
		...
		# Add to run queue
		self.run_queue.append(identifier_cur + "\n" + identifier_other) # For the node waiting on it
		self.arm_lock.release() # Release lock
		response.status = response.WAITING # If it isn't in completed then it isn't done
