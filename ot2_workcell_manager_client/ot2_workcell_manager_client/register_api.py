...
def register(self, type, name):
    ...
    if future.done():
        try:
            ...
            ):  # TODO: maybe add checks for valid status
                self.get_logger().error(
                    "Registration of type %s failed, retrying..." % type
                )
    ...
    if future.done():
        try:
            ...
            if response.status == response.ERROR or response.status == response.FATAL:
                self.get_logger().error(
                    "Deregistration failed of id: %s, retrying..." % self.id
                )
                return self.status["ERROR"]
            elif response.status == response.WARNING:
                self.get_logger().warn(
                    "Deregistration success of id: %s, but warning thrown by master"
                    % self.id
                )
