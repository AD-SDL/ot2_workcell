...
class OT2ProtocolManager(Node):
    ...
    def get_next_protocol(self):
        ...
        # Create client
        protocol_cli = self.create_client(
            Protocol, "/%s/%s/protocol" % (type, id)
            ...
            try:
                ...
                if response.status == response.ERROR:
                    self.get_logger().error(
                        "Error occured in protocol client %s for file %s"
                        % (id, response.file)
                    )
                    return self.status["ERROR"]  # Error
                    ...
                    self.get_logger().warning(
                        "Warning: File %s already exists on system %s"
                        % (response.file, id)
                    )
        ...
        try:
            ...
            if status == self.status["ERROR"]:
                self.get_logger().error(
                    "Error: protocol %s was not run successfully" % file_name
                )
                raise Exception
    ...
    def load_and_run(self, file):
        ...
        try:
            ...
            self.get_logger().info(
                "Module %s successfully loaded and attached to the program" % filepath
            )
        ...
        try:
            ...
            # Error
            self.get_logger().error(
                "Error occured when trying to run module %s: %r" % (filepath, e)
            )
            return self.status["ERROR"]
        else:
            self.get_logger().info(
                "Module %s successfully ran to completion" % filepath
            )
            return self.status["SUCCESS"]
